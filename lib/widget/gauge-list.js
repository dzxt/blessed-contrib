...
var blessed = require('blessed')
  , Node = blessed.Node
  , Canvas = require('blessed-contrib/lib/widget/canvas');
...
GaugeList.prototype.setGauges = function(gauges) {
  ...
  const leftStart = Math.max(...gauges.map(gg=>String(gg.stack[0].title).length||3))+1
  for (var i=0; i<gauges.length; i++) {
    this.setSingleGauge(gauges[i], i,leftStart);
  }

};

GaugeList.prototype.setSingleGauge = function(gauge, offset,leftStart) {
  ...
  const title =stack[0]?.title?String(stack[0]?.title):offset.toString()
  
  c.fillText(title, 0, offset*(this.options.gaugeHeight+this.options.gaugeSpacing));
